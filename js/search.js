(function () {
    var params = function () {
        var hash = {}, parts = window.location.search.substring(1).split('&');
    ...
    var filter = function (string) {
        var stop = [
          ...
          "let", "like", "likely", "may", "me", "might", "most", "must",
           ...
           "say", "says", "she", "should", "since", "so", "some",
        ...
        var regExp = new RegExp("(\\b(" + stop + ")\\b|[^a-z0-9 ])", "gi");

        return string.replace(regExp, "").replace(/  +/g, " ");
    };

    var words = filter('this is a test, I am testing to see if this works').split(" ");
    var output = [];
